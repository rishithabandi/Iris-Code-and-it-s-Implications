...
public class Agent
{
	...
	public void scanEye(String name, String path)
	{
		...
		if(this.sub_Subject.getIrisRadius() > 0 && this.sub_Subject.getPupilRadius() > 0)
		{
			...
			Functions.generateIrisCode(this.sub_Subject);
			...
			this.mem_Memory.addIdentity(new Identity(this.sub_Subject.getName(), this.sub_Subject.getIrisCode()));
		}
		else
		...
		if(this.sub_Subject.getIrisRadius() > 0 && this.sub_Subject.getPupilRadius() > 0)
		{
			...
			if(this.mem_Memory.size() > 0)
			{
				...
				for(int i = 0; i < this.mem_Memory.size(); i++)
				{
					...
					matchProbabilities[i] = Functions.matchIrisCode(this.sub_Subject, (Identity)this.mem_Memory.get(i));
